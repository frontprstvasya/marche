...
Drupal.behaviors.custom = {
    attach: function(context, settings) {
        ...
        const contactPhone = document.getElementById('edit-submitted-phone');
        const registrationPhone = document.getElementById('edit-field-uphone-und-0-value');
        ...
        contactPhone ? contactPhone.setAttribute('type', 'number') : false
        registrationPhone ? registrationPhone.setAttribute('type', 'number') : false
        ...
        /**
         * Language switcher in hidden menu and out of
         */
        const langSwitcher = document.getElementById('block-locale-language--2');
        const hiddenMenu = document.querySelector('.first-header-menu');
        const burger = document.querySelector('.icon-nm_10-burger-vertical');
        window.addEventListener('resize', () => {
            if (window.innerWidth <= 480) {
                hiddenMenu.append(langSwitcher);
                langSwitcher.style.display = "block";
            } else {
                burger.after(langSwitcher);
            }
